...
Seven chapters for people who write Go and never learned NumPy or
...
Chapters 1 to 4 use only the `tensor` package; 5 to 7 add `nn` and
`optim`. Every program runs in a few seconds on a laptop. Start with
