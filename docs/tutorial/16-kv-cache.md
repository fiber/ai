# 16. The KV cache: not recomputing the past

Chapter 15 ended with a confession. To produce one character, the model
...
```
go run ./examples/tutorial/16-kv-cache
```
...
content" vectors from chapter 13 — are exactly what they were. Only the
...
after the arithmetic — chapter 15's causal mask is a [128×128] matrix
...
in the wrong ones. The position scheme chosen in chapter 15 for one
...
That is the same wall chapter 14's small products ran into and the same
